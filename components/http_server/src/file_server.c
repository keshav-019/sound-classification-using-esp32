 ...
 static esp_err_t list_files_handler(httpd_req_t *req) {
     ...
     while ((entry = readdir(dir)) != NULL && file_count < MAX_FILES) {
         if (entry->d_type == DT_REG || entry->d_type == DT_DIR) {
             ...
             if (snprintf(full_path, sizeof(full_path), "%s/%s", SD_MOUNT_POINT, entry->d_name) >= sizeof(full_path)) {
                 continue;
             }
             ...
             if (stat(full_path, &st) == 0) {
                 ...
                 if (snprintf(json_response + strlen(json_response), 
                             LIST_BUFFER_SIZE - strlen(json_response),
                             "{\"name\":\"%s\",\"is_dir\":%d,\"size\":%d}",
                             entry->d_name,
                             entry->d_type == DT_DIR,
                             (int)st.st_size) >= (LIST_BUFFER_SIZE - strlen(json_response))) {
                     break;
                 }
